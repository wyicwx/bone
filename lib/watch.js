...
function startWatch() {
    ...
    if(!watcher) {
        watcher = sane(bone.status.base, '{!(node_modules)/**/*,!(node_modules)}');
        ...
        watcher.on('change', function(filePath) {
            watchEvent.emit('changed', filePath);
            watchEvent.emit('all', 'changed', filePath);
        });
        watcher.on('add', function(filePath) {
            watchEvent.emit('added', filePath);
            watchEvent.emit('all', 'added', filePath);
        });
        watcher.on('delete', function(filePath) {
            watchEvent.emit('deleted', filePath);
            watchEvent.emit('all', 'deleted', filePath);
        });
    }
}
