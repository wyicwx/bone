...
// file path src 
function FileSystem(base, options) {
    options || (options = {});
...
FileSystem.prototype.createReadStream = function(filePath, options) {
    var fileSystem = this;

    options || (options = {});
    options.act = !options.act ? [] : (_.isArray(options.act) ? options.act : [options.act]);
    ...
    if(this.globalAct) {
        if(_.isArray(this.globalAct)) {
            options.act = options.act.concat(this.globalAct);
        } else {
            options.act.push(this.globalAct);
        }
    }
    ...
    var readStream = new ReadStream(filePath, {
            ...
            onEnd: function() {
                if(fileSystem._readFiles) {
                    fileSystem._readFiles = fileSystem._readFiles.concat(readStream.actsDependent);
                }
            }
        }, options.act);
...
FileSystem.event = new EventEmitter();
...
FileSystem.setTraceTree = function(file, dependencies) {
    ...
    this.event.emit('setTraceTree', dependencies);
};
...
FileSystem.getFs = function(options) {
    if(options) {
        return new FileSystem(bone.status.base, options);
    } else {
