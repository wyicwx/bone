...
ReadStream.prototype._read = function() {
    ...
    var objStream = through.obj(function(buffer, encoding, callback) {
        var data = {
            ...
            cacheable: true,
            dependency: [readStream.originSource],
            fs: FileSystem.getFs({captureFile: true})
        };
        callback(null, data);
    });
...
ReadStream.prototype._handlerAct = function() {
    ...
    var handler = function(act) {
        ...
        readStream._readStream = readStream._readStream.pipe(act.getThroughStream());
    }
