...
function _handlerAct(acts, origin, self, fs) {
	...
	if(self.isVirtual) {
		...
		self.sourceStream = function() {
			if(cache.cached(self.path)) {
				...
				origin = _handlerAct(acts, origin, self, fs);
...
function ReadStream(file, option) {
	...
	this.trackStack = [];
	// 附加act
	this.acts = !option.act ? [] : (_.isArray(option.act) ? option.act : [option.act]);
	// 解析文件链
...
ReadStream.prototype.toStream = function() {
	...
	stream = _handlerAct(this.acts, stream, this, bonefs);
	return stream;
};
