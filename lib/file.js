...
File.setup = function(file) {
	var pathResolve = function() {
		return bone.fs.pathResolve.apply(bone.fs, arguments);
	}
...
FileFormat.prototype.glob = function() {
	...
	if(result.length) {
		...
		// warn
		bone.log.warn('Not exists: '+this.source);
	}
};
...
FileFormat.prototype.register = function(source) {
	if(!this.isGlobSyntax) {
		source = this.source;
		if(!bone.fs.existFile(source)) {
			bone.log.warn('Not exists: '+source);
			return;
		}
	}
