...
File.prototype.define = function(p) {
	return new File(p, {
		parent: path.join(this.parentPath, this.dest)
	});
};
...
function setupFile(file) {
	...
	if(file.source) {
		...
		var fullDestPath = bone.fs.pathResolve(file.dest, file.parentPath);
		...
		if(isBatch) {
			...
			if(!bone.fs.exists(fullSourcePath)) {
				// warn
				console.log('not exists:'+fullSourcePath);
			} else {
