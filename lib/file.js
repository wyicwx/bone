...
function File(destination, options) {
    options || (options = {});
    this.parentPath = options.parent || ''; // raw path
    this.destination = destination || '';
    this.source = null;
    this.renameFn = null;
    this.dirFn = null;
    this.cwdPath = options.cwd || '';
    this.acts = [];
    this.globalActs = [];
...
File.prototype.act = function(fn, options) {
    options || (options = {});
    if(!this.source) {
...
FileFormat.prototype.register = function(source) {
    ...
    FileSystem.register(dest, {
        ...
        globalActs: _.clone(this.file.globalActs),
...
FileFormat.prototype.rename = function(source, dest) {
    ...
    if(_.isFunction(file.renameFn)) {
        ...
        if(renameOption.ext && _.isString(renameOption.ext)) {
            if(renameOption.ext.indexOf('.') != 0) {
                fileProperty.ext = '.'+renameOption.ext;
            } else {
                fileProperty.ext = renameOption.ext;
            }
        } else if(renameOption.extTransport && _.isObject(renameOption.extTransport)) {
            if(fileProperty.ext in renameOption.extTransport) {
                fileProperty.ext = renameOption.extTransport[fileProperty.ext];
            }
        }
