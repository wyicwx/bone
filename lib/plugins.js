...
function Plugins(processor, options) {
    ...
    this._globalEnable = options.globalEnable;
...
Plugins.prototype.getThroughStream = function() {
    ...
    var stream = through.obj(function(data, encoding, callback) {
        ...
        runtime.addDependency(data.dependency);
        // filter processor 
        ...
        plugins.processor.call(runtime, data.buffer, encoding, function(error, content) {
            ...
            data.buffer = buffer;
            data.dependency = _.uniq(data.dependency.concat(runtime._dependencyList));
...
Runtime.prototype.getDependency = function() {
    return _.clone(this._dependencyList);
};
...
module.exports.registerAction = function(moduleName) {
    ...
    if(_.isFunction(mod)) {
        ...
        if(!mod.act) {
            log.error('require', 'module "'+moduleName+'" is no design for bone!');
        } else {
            act = mod.act;
        }
    }
